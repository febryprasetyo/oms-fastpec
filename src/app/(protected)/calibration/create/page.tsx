...
export default function CreateCalibrationPage() {
  ...
  const createDraft = async () => {
    ...
    try {
      ...
      toast.success("Draft calibration berhasil dibuat.");
  ...
  return <div className="mx-auto max-w-5xl space-y-6 p-6">
    <div><h1 className="text-2xl font-bold">Buat Calibration Draft</h1><p className="text-sm text-muted-foreground">Officer: {officerName}. Metadata report dan standard akan dibuat oleh server.</p></div>
    <Card><CardHeader><CardTitle>Informasi Calibration</CardTitle></CardHeader><CardContent className="grid gap-4 md:grid-cols-2">
      <div className="space-y-2 md:col-span-2"><Label>Station</Label><Input value={stationSearch} onChange={(event) => { setStationSearch(event.target.value); setStationId(""); }} placeholder="Cari station..." />
    ...
    <Card><CardHeader><CardTitle>Parameter</CardTitle></CardHeader><CardContent className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">{parametersLoading ? <p className="text-sm text-muted-foreground">Memuat master parameter...</p> : parameters.map((parameter) => { const selected = parameterIds.includes(parameter.id); return <button key={parameter.id} type="button" onClick={() => toggleParameter(parameter.id)} className={`flex items-center gap-3 rounded-lg border p-3 text-left text-sm ${selected ? "border-primary bg-primary/5" : "hover:bg-muted"}`}>{selected ? <Check className="h-4 w-4 text-primary" /> : <X className="h-4 w-4 text-muted-foreground" />}<span>{parameter.name}</span></button>; })}</CardContent></Card>
    <div className="flex justify-end gap-3"><Button variant="outline" onClick={() => router.back()}>Batal</Button><Button disabled={createMutation.isPending} onClick={() => void createDraft()}>{createMutation.isPending ? "Membuat..." : "Buat Draft"}</Button></div>
  </div>;
}
