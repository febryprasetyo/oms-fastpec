...
export const NotesEditor = ({ value, onChange }: NotesEditorProps) => {
  ...
  const action = (label: string, icon: ReactNode, run: () => void, active = false) => <Button type="button" variant={active ? "secondary" : "outline"} size="icon" className="h-8 w-8" aria-label={label} onClick={run}>{icon}</Button>;
  return <div className="space-y-2"><Label className="text-sm font-semibold">Notes & Maintenance Actions</Label><div className="overflow-hidden rounded-md border bg-background"><div className="flex flex-wrap gap-1 border-b bg-muted/40 p-2">{action("Bold", <Bold className="h-4 w-4" />, () => editor.chain().focus().toggleBold().run(), editor.isActive("bold"))}{action("Italic", <Italic className="h-4 w-4" />, () => editor.chain().focus().toggleItalic().run(), editor.isActive("italic"))}{action("Underline", <UnderlineIcon className="h-4 w-4" />, () => editor.chain().focus().toggleUnderline().run(), editor.isActive("underline"))}{action("Strike", <Strikethrough className="h-4 w-4" />, () => editor.chain().focus().toggleStrike().run(), editor.isActive("strike"))}{action("Bullet list", <List className="h-4 w-4" />, () => editor.chain().focus().toggleBulletList().run(), editor.isActive("bulletList"))}{action("Numbered list", <ListOrdered className="h-4 w-4" />, () => editor.chain().focus().toggleOrderedList().run(), editor.isActive("orderedList"))}{action("Undo", <Undo2 className="h-4 w-4" />, () => editor.chain().focus().undo().run())}{action("Redo", <Redo2 className="h-4 w-4" />, () => editor.chain().focus().redo().run())}</div><EditorContent editor={editor} /></div></div>;
};
