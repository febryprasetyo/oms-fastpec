...
import {
  addPengajuanInternet,
  editPengajuanInternet,
...
const formSchema = z.object({
  ...
  nama_paket: z.string({ required_error: "Nama paket harus diisi" }),
  masa_aktif: z.coerce.number({ required_error: "Masa aktif harus diisi" }),
...
type Props = {
  ...
  data?: PengajuanInternetData;
};

export default function PengajuanInternetForm({
  action,
  setIsOpen,
  data,
}: Props) {
  ...
  const mutation = useMutation({
    mutationFn: async (formData: z.infer<typeof formSchema>) => {
      const payload: PengajuanInternetRequest = {
        ...formData,
      ...
      if (action === "edit") {
        const res = await editPengajuanInternet(
        ...
        const res = await addPengajuanInternet(payload, accessToken as string);
        if (res?.success === false) throw new Error(res.message);
        return res;
      }
    },
    ...
    onSuccess: () => {
      ...
      queryClient.invalidateQueries({ queryKey: ["pengajuan-internet"] });
      setIsOpen(false);
    },
  });
        ...
        <FormField
          ...
          render={({ field }) => (
            <FormItem>
              ...
              <FormControl>
                <Input placeholder="Contoh: Sakti 1" {...field} />
              </FormControl>
              <FormMessage />
            </FormItem>
          )}
        ...
        <FormField
          ...
          render={({ field }) => (
            <FormItem>
              ...
              <FormControl>
                <Input type="number" placeholder="Contoh: 30" {...field} />
              </FormControl>
              <FormMessage />
            </FormItem>
          )}
